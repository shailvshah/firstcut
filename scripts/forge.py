...
def h(text: str) -> str:   return f"{BOLD}{text}{RESET}"
def dim(text: str) -> str: return f"{DIM}{text}{RESET}"
def ok(text: str) -> str:  return f"{GREEN}✓{RESET} {text}"
def info(text: str) -> str:return f"{BLUE}→{RESET} {text}"
def warn(text: str) -> str:return f"{AMBER}!{RESET} {text}"
...
SKILLS = [
    ...
    ("request-refactor-plan",  "Refactor-before-implement — plan before changing existing code"),
    ("interface-design",       "Interface-before-class enforcer"),
    ("implementation-simplicity", "Complexity ≤ 3, pure functions, no premature abstraction"),
]
...
def prompt(question: str, default: str, choices: Optional[list[str]] = None) -> str:
...
def prompt_multi(question: str, options: list[tuple[str, str]], default_all: bool = True) -> list[str]:
    print(f"\n{question}")
    for i, (key, desc) in enumerate(options, 1):
        print(f"  {dim(str(i)+'.')} {h(key)} — {desc}")
    default_label = "all" if default_all else "none"
    raw = input(f"\n  Enter numbers to toggle off (comma-separated), or press Enter to keep {default_label}: ").strip()
    if not raw:
...
def step2_stack(cfg: ForgeConfig) -> None:
    ...
    cfg.ci = ["github-actions", "gitlab-ci"] if ci_raw == "both" else ([] if ci_raw == "none" else [ci_raw])
...
def step4_skills(cfg: ForgeConfig) -> None:
    ...
    cfg.include_docs_submodule = confirm("Include docs/ as a submodule scaffold?", default=True)
...
def write_claude_md(cfg: ForgeConfig) -> None:
    skills_list = "\n".join(
        f"- **{s}** → `skills/{s}/SKILL.md`" for s in cfg.skills
    )
    write(cfg.dest / "CLAUDE.md", f"""# {cfg.slug}
...
def _install_cmd(cfg: ForgeConfig) -> str:
    return {"uv": "uv sync", "poetry": "poetry install",
            "pnpm": "pnpm install", "npm": "npm install",
            "cargo": "cargo build", "go mod": "go mod download"}.get(cfg.pkg_manager, "install deps")


def _dev_cmd(cfg: ForgeConfig) -> str:
    if cfg.pkg_manager == "poetry":
        fw_map = {"fastapi": "poetry run uvicorn src.main:app --reload",
                  "mkdocs": "poetry run mkdocs serve"}
        return fw_map.get(cfg.framework, "poetry run python -m src.main")
    m = {"fastapi": "uvicorn src.main:app --reload", "nextjs": "pnpm dev",
         "hono": "pnpm dev", "gin": "go run ./cmd/server", "mkdocs": "mkdocs serve"}
    return m.get(cfg.framework, f"{cfg.pkg_manager} run dev")


def _test_cmd(cfg: ForgeConfig) -> str:
    return {"uv": "uv run pytest --cov=src --cov-fail-under=95",
            "poetry": "poetry run pytest --cov=src --cov-fail-under=95",
            "pnpm": "pnpm test", "go mod": "go test ./... -cover",
            "cargo": "cargo test"}.get(cfg.pkg_manager, "run tests")


def _lint_cmd(cfg: ForgeConfig) -> str:
    return {"uv": "uv run ruff check . && uv run black .",
            "poetry": "poetry run ruff check . && poetry run black .",
            "pnpm": "pnpm lint", "go mod": "golangci-lint run",
            "cargo": "cargo clippy"}.get(cfg.pkg_manager, "run lint")


def _typecheck_cmd(cfg: ForgeConfig) -> str:
    return {"uv": "uv run mypy src/", "poetry": "poetry run mypy src/",
            "pnpm": "pnpm typecheck",
            "go mod": "go vet ./..."}.get(cfg.pkg_manager, "run typecheck")
...
def _build_skills(cfg: ForgeConfig) -> dict[str, str]:
    ...
    fw   = cfg.framework
    ext  = cfg.ext
    test_ext = {"py": "_test.py", "ts": ".test.ts", "go": "_test.go", "rs": "_test.rs"}.get(ext, f".test.{ext}")
    runner = {"py": "pytest", "ts": "vitest", "go": "go test", "rs": "cargo test"}.get(ext, "test runner")
...
def write_github_ci(cfg: ForgeConfig) -> None:
    ...
    write(cfg.dest / ".github" / "workflows" / "ci.yml", f"""name: CI
...
""")
    ...
    write(cfg.dest / ".github" / "workflows" / "release.yml", f"""name: Release
...
def write_gitlab_ci(cfg: ForgeConfig) -> None:
    ...
    write(cfg.dest / ".gitlab-ci" / "ci.yml", f"""image: {image}
...
def write_docs(cfg: ForgeConfig) -> None:
    docs = cfg.dest / "docs"
    write(docs / "domain" / "glossary.md", f"""# Domain glossary — {cfg.slug}
...
""")
    ...
    write(docs / "CONTRIBUTING.md", f"""# Contributing to {cfg.slug}
...
""")
    ...
    write(docs / "ADR" / "0001-first-principles-sdlc.md", f"""# ADR 0001 — First-principles SDLC
...
def write_agents_md(cfg: ForgeConfig) -> None:
    ...
    write(cfg.dest / "AGENTS.md", f"""# {cfg.slug}
...
def write_readme(cfg: ForgeConfig) -> None:
    write(cfg.dest / "README.md", f"""# {cfg.slug}
...
def write_lang_config(cfg: ForgeConfig) -> None:
    if cfg.language == "python":
        if cfg.pkg_manager == "poetry":
            write(cfg.dest / "pyproject.toml", f"""[tool.poetry]
name = "{cfg.slug}"
version = "0.1.0"
...
""")
            ...
            write(cfg.dest / "pyproject.toml", f"""[project]
name = "{cfg.slug}"
version = "0.1.0"
description = "{cfg.description}"
...
""")
        ...
        write(cfg.dest / "package.json", json.dumps({
            "name": cfg.slug,
            "version": "0.1.0",
            "description": cfg.description,
            "license": cfg.license,
            "scripts": {
                "dev": "next dev" if cfg.framework == "nextjs" else "tsx src/index.ts",
        ...
        write(cfg.dest / "tsconfig.json", json.dumps({
            "compilerOptions": {
                "target": "ES2022", "module": "NodeNext",
                "moduleResolution": "NodeNext",
                "strict": True, "noUncheckedIndexedAccess": True,
                "exactOptionalPropertyTypes": True, "outDir": "dist",
                "rootDir": "src"
            },
            "include": ["src/**/*"],
        ...
        write(cfg.dest / "pnpm-workspace.yaml", "packages:\n  - 'apps/*'\n  - 'packages/*'\n")
        write(cfg.dest / "turbo.json", json.dumps({
        ...
        write(cfg.dest / "packages" / "config" / "tsconfig" / "base.json",
              json.dumps(base_tsconfig, indent=2))

    elif cfg.language == "go":
        write(cfg.dest / "go.mod", f"""module github.com/{cfg.org}/{cfg.slug}
...
def write_project_structure(cfg: ForgeConfig) -> None:
    ...
    dispatch = {
        ...
        ("frontend",       "typescript"): _scaffold_nextjs if cfg.framework == "nextjs" else _scaffold_ts_frontend,
...
def _scaffold_python_backend(cfg: ForgeConfig) -> None:
    ...
    write(d / "src" / "main.py", f'"""FastAPI application factory — wires DI and mounts routers."""\n'
          'from __future__ import annotations\n'
    ...
    write(d / "src" / "domain" / "__init__.py",
          _py_init("Pure business logic. Zero infrastructure imports allowed here.\n"
                   "Governed by: skills/domain-model/SKILL.md"))
    write(d / "src" / "domain" / "models.py",
    ...
    write(d / "src" / "domain" / "repositories.py",
    ...
    write(d / "src" / "application" / "__init__.py",
          _py_init("Use cases — orchestrate domain. No HTTP, no SQL imports."))
    write(d / "src" / "application" / "commands.py",
    ...
    write(d / "src" / "application" / "queries.py",
    ...
    write(d / "src" / "infrastructure" / "__init__.py",
          _py_init("Adapters — concrete implementations of domain interfaces."))
    write(d / "src" / "infrastructure" / "config.py",
          ...
          'settings = Settings()\n')
    write(d / "src" / "infrastructure" / "db.py",
    ...
    write(d / "tests" / "unit" / "domain" / "test_models.py",
    ...
    write(d / "tests" / "integration" / "test_api.py",
...
def _scaffold_python_tooling(cfg: ForgeConfig) -> None:
    ...
    write(d / "src" / "models" / "__init__.py",
          _py_init("Data models — Pydantic config, typed inputs/outputs."))
    write(d / "src" / "models" / "config.py",
          '"""Configuration models — loaded from env or config file."""\n'
          'from __future__ import annotations\n'
          'from pydantic import BaseModel\n\n\n'
          'class Config(BaseModel):\n'
          '    verbose: bool = False\n')
    write(d / "src" / "adapters" / "__init__.py",
          _py_init("Infrastructure adapters — filesystem, network, subprocess. Isolated here."))
    write(d / "src" / "adapters" / "filesystem.py",
    ...
    write(d / "tests" / "__init__.py", "")
    write(d / "tests" / "unit" / "__init__.py", "")
    write(d / "tests" / "unit" / "test_core.py",
          ...
          '    assert True\n')
    write(d / "tests" / "integration" / "__init__.py", "")
    write(d / "tests" / "integration" / "test_cli.py",
...
def _scaffold_ts_backend(cfg: ForgeConfig) -> None:
    ...
    write(d / "src" / "domain" / "repositories.ts",
          ...
          "// }\n")
    write(d / "src" / "application" / "commands.ts",
    ...
    write(d / "src" / "application" / "queries.ts",
          "// Query handlers — read-side use cases.\n\n"
          "// Add query handlers below\n")
    write(d / "src" / "infrastructure" / "config.ts",
...
def _scaffold_nextjs(cfg: ForgeConfig) -> None:
    ...
    write(d / "src" / "app" / "layout.tsx",
          ...
          "  )\n"
          "}\n")
    write(d / "src" / "app" / "page.tsx",
          f"export default function Home() {{\n"
          f"  return <main><h1>{s}</h1></main>\n"
          f"}}\n")
    write(d / "src" / "components" / "ui" / "button.tsx",
...
def _scaffold_ts_frontend(cfg: ForgeConfig) -> None:
    ...
    write(d / "src" / "components" / "index.ts",
...
def _scaffold_monorepo(cfg: ForgeConfig) -> None:
    ...
    # apps/api
    write(d / "apps" / "api" / "src" / "index.ts",
          "// API application entry point\n")
    write(d / "apps" / "api" / "src" / "index.test.ts",
          "import { describe, it, expect } from 'vitest'\n"
          "describe('api', () => { it('starts', () => expect(true).toBe(true)) })\n")
    write(d / "apps" / "api" / "package.json", json.dumps({
        "name": f"@{s}/api", "version": "0.1.0", "private": True,
        "scripts": {"dev": "tsx src/index.ts", "build": "tsc", "test": "vitest run"},
        "devDependencies": {"typescript": "^5.4", "vitest": "^1.6", "tsx": "^4"},
    }, indent=2))
    write(d / "apps" / "api" / "tsconfig.json", json.dumps({
    ...
    # apps/web
    write(d / "apps" / "web" / "src" / "app" / "page.tsx",
          f"export default function Home() {{ return <main><h1>{s}</h1></main> }}\n")
    write(d / "apps" / "web" / "package.json", json.dumps({
        "name": f"@{s}/web", "version": "0.1.0", "private": True,
        "scripts": {"dev": "next dev", "build": "next build", "test": "vitest run"},
        "devDependencies": {"next": "^14", "typescript": "^5.4", "vitest": "^1.6"},
    }, indent=2))
    write(d / "apps" / "web" / "tsconfig.json", json.dumps({
        "extends": "../../packages/config/tsconfig/base.json",
    ...
    # packages/ui
    write(d / "packages" / "ui" / "src" / "index.ts",
          "// Shared UI component library — design system primitives.\n")
    write(d / "packages" / "ui" / "package.json", json.dumps({
        "name": f"@{s}/ui", "version": "0.1.0",
        "main": "./dist/index.js", "types": "./dist/index.d.ts",
        "scripts": {"build": "tsc", "test": "vitest run"},
    }, indent=2))

    # packages/shared
    write(d / "packages" / "shared" / "src" / "index.ts",
          "// Shared types and utilities — names from docs/domain/glossary.md.\n")
    write(d / "packages" / "shared" / "package.json", json.dumps({
        "name": f"@{s}/shared", "version": "0.1.0",
        "main": "./dist/index.js", "types": "./dist/index.d.ts",
        "scripts": {"build": "tsc", "test": "vitest run"},
    }, indent=2))
...
def _scaffold_go_backend(cfg: ForgeConfig) -> None:
    ...
    write(d / "internal" / "domain" / "model.go",
          f"// Package domain — pure business logic.\n"
          f"// Zero imports from infrastructure, api, or framework packages.\n"
          f"// Names must appear in docs/domain/glossary.md.\n"
          f"package domain\n\n"
          f"// Add domain entities and value objects below\n")
    write(d / "internal" / "domain" / "events.go",
          "// Domain events — past-tense facts emitted from aggregates.\n"
          "package domain\n\n"
          "// Add event types below\n")
    write(d / "internal" / "domain" / "repository.go",
    ...
    write(d / "internal" / "application" / "service.go",
          "// Application services — use-case orchestration.\n"
          "// No HTTP, no SQL — only domain types and repository interfaces.\n"
          "package application\n\n"
          "// Add use-case service types below\n")

    write(d / "internal" / "infrastructure" / "db" / "postgres.go",
          "// Concrete repository implementations.\n"
          "// Implements domain repository interfaces.\n"
          "package db\n\n"
          "// Add concrete repository structs below\n")
    write(d / "internal" / "infrastructure" / "config" / "config.go",
          f'package config\n\nimport "os"\n\n'
    ...
    write(d / "internal" / "api" / "routes.go",
          ...
          f"\treturn r\n"
          f"}}\n")
    write(d / "internal" / "api" / "middleware.go",
          "// Middleware — logging, auth, recovery.\n"
          "package api\n")

    write(d / "pkg" / "errors" / "errors.go",
    ...
    write(d / "tests" / "integration" / "api_test.go",
...
def _scaffold_go_tooling(cfg: ForgeConfig) -> None:
    ...
    write(d / "cmd" / s / "main.go",
          f"package main\n\n"
          f'import "github.com/{org}/{s}/internal/cli"\n\n'
          f"func main() {{\n"
          f"\tcli.Execute()\n"
          f"}}\n")
    write(d / "internal" / "cli" / "root.go",
          ...
          f"\t_ = rootCmd.Execute()\n"
          f"}}\n")
    write(d / "internal" / "core" / "logic.go",
          "// Pure business logic — no IO, no CLI framework imports.\n"
          "package core\n\n"
          "// Add pure functions here\n")
    write(d / "internal" / "adapters" / "filesystem.go",
          "// Filesystem adapter — all fs operations isolated here.\n"
          "package adapters\n\n"
          "// Add filesystem helpers here\n")
    write(d / "pkg" / "types" / "types.go",
...
def _scaffold_rust_backend(cfg: ForgeConfig) -> None:
    ...
    write(d / "src" / "main.rs",
          ...
          "}\n")
    write(d / "src" / "domain" / "mod.rs",
          "// Pure business logic — zero infrastructure imports.\n"
          "pub mod models;\n"
          "pub mod events;\n"
          "pub mod repositories;\n")
    write(d / "src" / "domain" / "models.rs",
          "// Domain entities and value objects.\n")
    write(d / "src" / "domain" / "events.rs",
          "// Domain events — past-tense (OrderPlaced, UserRegistered).\n")
    write(d / "src" / "domain" / "repositories.rs",
          "// Repository traits — no DB types.\n")
    write(d / "src" / "application" / "mod.rs",
    ...
    write(d / "src" / "infrastructure" / "mod.rs",
          "// Adapters — concrete implementations.\n"
          "pub mod config;\n"
          "pub mod db;\n")
    write(d / "src" / "infrastructure" / "config.rs",
          ...
          "    }\n"
          "}\n")
    write(d / "src" / "infrastructure" / "db.rs",
          "// Concrete repository implementations.\n")
    write(d / "src" / "api" / "mod.rs",
...
def _scaffold_terraform(cfg: ForgeConfig) -> None:
    ...
    for env in ("dev", "staging", "prod"):
        write(d / "stacks" / env / "main.tf",
              ...
              f'}}\n')
        write(d / "stacks" / env / "variables.tf",
    ...
    for mod in ("network", "compute", "storage"):
        write(d / "modules" / mod / "main.tf",
              f'# {mod.capitalize()} module\n')
        write(d / "modules" / mod / "variables.tf",
              f'# {mod.capitalize()} module inputs\n')
        write(d / "modules" / mod / "outputs.tf",
              f'# {mod.capitalize()} module outputs\n')

    write(d / "scripts" / "deploy.sh",
          '#!/usr/bin/env bash\n'
          'set -euo pipefail\n\n'
          'ENV=${1:-dev}\n'
          'cd "stacks/$ENV"\n'
...
def _scaffold_mkdocs(cfg: ForgeConfig) -> None:
    ...
    write(d / "docs" / "index.md",
          f"# {s}\n\n{cfg.description}\n")
    write(d / "docs" / "getting-started.md",
          f"# Getting started\n\n## Installation\n\n```bash\n# Add install steps here\n```\n")
    write(d / "docs" / "api" / "reference.md",
...
def _scaffold_docusaurus(cfg: ForgeConfig) -> None:
    ...
    write(d / "docs" / "intro.md",
          f"# {s}\n\n{cfg.description}\n")
    write(d / "docs" / "getting-started.md",
          "# Getting started\n\n## Install\n\n```bash\n# Add steps here\n```\n")
    write(d / "src" / "pages" / "index.tsx",
          f"export default function Home() {{\n"
          f"  return <main><h1>{s}</h1></main>\n"
          f"}}\n")
...
def init_git(cfg: ForgeConfig) -> None:
    ...
    subprocess.run(
        ["git", "commit", "-m", f"chore: initial scaffold via firstcut\n\nType: {cfg.project_type}\nStack: {cfg.language}/{cfg.framework}"],
        cwd=dest, check=True, capture_output=True,
        env={**os.environ, "GIT_AUTHOR_NAME": cfg.org, "GIT_AUTHOR_EMAIL": f"forge@{cfg.org}.com",
             "GIT_COMMITTER_NAME": cfg.org, "GIT_COMMITTER_EMAIL": f"forge@{cfg.org}.com"}
    )
...
def print_summary(cfg: ForgeConfig) -> None:
    ...
    print(f"  {dim('Type')}       {cfg.project_type} · {cfg.language} / {cfg.framework}")
    ...
    print(f"  3. pip install pre-commit && pre-commit install --hook-type commit-msg && pre-commit install")
